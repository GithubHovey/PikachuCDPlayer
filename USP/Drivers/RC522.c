...
uint8_t UID[8],Temp[4];
uint8_t ntag_txdata[16];
uint8_t ntag_rxdata[16];

// uint8_t UI0[4]={0xDC,0x9F,0x29,0xA9};		 	//卡0ID号
// uint8_t UI1[4]={0x31,0x55,0x2F,0x1A};	 		//卡1ID号
...
char PcdSelect2(unsigned char *pSnr)
{
    char status;
    unsigned char i;
    unsigned int  unLen;
